scoreboard players add @a[scores={top_y_0=0..63}] top_y_0 1
execute @a[scores={top_y_0=63..,top_y_0=!63}] ~~~playsound random.levelup
scoreboard players reset @a[scores={top_y_0=63..,top_y_0=!63}] top_y_0
#function_02_chanyv
...
tellraw @a[scores={godmode=1},tag=!tell] {"rawtext":[{"text":"§l§f2.§e我们将会在您进入开发者模式的那一瞬间将您快捷物品栏的前§f4§e格替换为相应的功能物品§f，§e他们用于执行相应的开发者模式下才能执行的功能§f。§e若您因为疏忽导致储存于这些物品栏的物品丢失§f，§e我们概不负责§f！"}]}
...
tellraw @a[scores={godmode=2}] {"rawtext":[{"text":"§l§f[§d钓鱼竿§f]§7-§b自§b杀§f。§b扔出即可自§b杀§f，§b死亡时不会自§b杀§f。"}]}
...
tellraw @a[tag=godmode_die,m=!1] {"rawtext":[{"text":"§c已成功自§c杀§f！"}]}
