...
- [第三章 • `玩家传送`](#第三章-•-玩家传送)
- [第四章 • `地皮`](#第四章-•-地皮)
   - [第一节 功能简介](#e7acace4b880e88a82-e58a9fe883bde7ae80e4bb8b-1)
   - [第二节 操作指南](#e7acace4ba8ce88a82-e6938de4bd9ce68c87e58d97-1)
...
`4`|修复|`omg [修复\|4] <修复半径>`|`修复半径` 必须是一个有效的 `数字` _[需要更多信息]_|根据 `Backup` 文件夹中所记的存档数据，恢复 `命令执行者` 周围指定 `修复半径` 内的 `区块` 但损失 `方块实体` 数据|**需要 `授权` 才能使用此命令**|`omg 修复 10`<br/>`omg 1 30`
...
## 第四节 必要说明
`Omg` 菜单的使用前提是玩家 `HappyLove666` 处于 `在线` 状态，当该 `机器人` 为下线状态时则不能使用。
...
## 第二节 功能
- `随机传送` _[需要更多信息]_
   - 用于将玩家随机传送到一个坐标，开荒跑图必备
- `玩家传送`
   - 该传送点可以将你传送至传送屋，随后你可以传送至某个玩家（详情见第三章）
- `商店/抽奖机`
   - 该传送点可以将你传送至 `商店`
...
- `生存区/地皮`
   ...
   - `生存区传送点` _[可能需要更新]_
      ...
      - `山谷传送点`
   - `地皮区传送点`
      - 可以将你传送至 `地皮小屋`，在那里你可以购买 `地皮` 和回到自己的 `地皮`（详情见第四章）
